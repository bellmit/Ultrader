...
export default class EditStrategyComp extends React.Component {
  ...
  loadData() {
    ...
    Promise.all(promises)
      .then(responses => {
        ...
        var ruleOptions = responses[0].data.map(rule => {
        ...
        var strategyOptions = strategies.map(strategy => {
          return { label: strategy.name, value: strategy.id };
        });
        this.setSelectedValues(ruleOptions, strategyOptions);
      })
      .catch(error => {
        alertError(error);
      });
  ...
  setSelectedValues(ruleOptions, strategyOptions) {
    ...
    for (var i = 0; i < formulaParts.length; i++) {
      ...
      var isStrategy = formulaPart.includes("S");
      formulaPart = formulaPart.replace("S", "");
      var r = /\d+/;
      var id = formulaPart.match(r);
      var operator = formulaPart.replace(id, "");
      var strategyComponent = {};
      if (isStrategy) {
        var selectedStrategyOption = strategyOptions.find(e => e.value == id);
        strategyComponent = {
          selectedType: componentTypeOptions[1].value,
          selectedComponentValue: selectedStrategyOption
            ? selectedStrategyOption.value
            : "",
          selectedTypeOption: componentTypeOptions[1],
          selectedComponentValueOption: selectedStrategyOption
            ? selectedStrategyOption
            : {}
        };
      } else {
        var selectedRuleOption = ruleOptions.find(e => e.value == id);
        strategyComponent = {
          selectedType: componentTypeOptions[0].value,
          selectedComponentValue: selectedRuleOption
            ? selectedRuleOption.value
            : "",
          selectedTypeOption: componentTypeOptions[0],
          selectedComponentValueOption: selectedRuleOption
            ? selectedRuleOption
            : {}
        };
      }
      ...
      strategyComponents[i] = strategyComponent;
